...
smA_t1.init_timer()
axis_rx.write(smA_t1, strToInt("{AMHeader,0xCC,0x02,8,0,0x41,0}"))
axis_rx.write(smA_t1, strToInt("{AMToken,0x1}"), callTB=2)
short_message_A.add_thread(smA_t1)

smA_t2 = Thread()
axis_kernel_out.read(smA_t2, strToInt("{KernelHeader,0x41,0xCC,8,1}"), tdest=2)
...
smB_t2.init_timer()
axis_rx.write(smB_t2, strToInt("{AMHeader,0x10,0x01,16,0x1,1,1}"))
axis_rx.write(smB_t2, strToInt("{AMToken,0x0}"))
axis_rx.write(smB_t2, 4, tlast=1)
axis_rx.write(smB_t2, strToInt("{AMHeader,0x10,0x01,16,0x1,1,1}"))
axis_rx.write(smB_t2, strToInt("{AMToken,0x0}"))
axis_rx.write(smB_t2, 1, tlast=1)
axis_rx.write(smB_t2, strToInt("{AMHeader,0x10,0x01,8,0,1,0}"))
axis_rx.write(smB_t2, strToInt("{AMToken,0x0}"), tlast=1)
axis_rx.write(smB_t2, strToInt("{AMHeader,0x10,0x00,16,0x1,1,1}"))
axis_rx.write(smB_t2, strToInt("{AMToken,0x0}"))
...
axis_tx.read(smB_t3, strToInt("{AMHeader,0x1,0x010,8,0x0,0x41,0}"))
axis_tx.read(smB_t3, strToInt("{AMToken,0x0}"))
axis_tx.read(smB_t3, strToInt("{AMHeader,0x1,0x010,8,0x0,0x41,0}"))
axis_tx.read(smB_t3, strToInt("{AMToken,0x0}"))
axis_tx.read(smB_t3, strToInt("{AMHeader,0x1,0x010,8,0x0,0x41,0}"))
axis_tx.read(smB_t3, strToInt("{AMToken,0x0}"))
axis_tx.read(smB_t3, strToInt("{AMHeader,0x0,0x10,8,0x0,0x41,0}"))
axis_tx.read(smB_t3, strToInt("{AMToken,0x0}"))
...
axis_handler.read(smB_t4, strToInt("{AMHeader,0x10,0x01,16,0x1,0x1,1}"))
...
axis_handler.read(smB_t4, strToInt("{AMHeader,0x10,0x00,16,0x1,0x1,1}"))
...
axis_rx.writes(mmA_t1, [
    {"tdata": strToInt("{AMHeader,0xAA,0x2,0x808,0x0,0x2,0}")},
...
mmA_t3 = Thread()
axis_tx.read(mmA_t3, strToInt("{AMHeader,0x2,0xAA,8,0x0,0x41,0}"))
...
mmC_t1.init_timer()
axis_rx.write(mmC_t1, strToInt("{AMHeader,0x10,0,0x30,0,0x42,0}"), callTB=1),
axis_rx.write(mmC_t1, strToInt("{AMToken,0x0}")),
axis_rx.write(mmC_t1, 0x10, tlast=1, callTB=1)
medium_message_C.add_thread(mmC_t1)

mmC_t2 = Thread()
axis_tx.read(mmC_t2, strToInt("{AMHeader,0,0x10,0x30,0,0x22,0}"))
...
axis_rx.writes(lmA_t1, [
    {"tdata": strToInt("{AMHeader,0xAA,0x2,0x88,1,0x5,2}")},
...
lmA_t2 = Thread()
axis_handler.read(lmA_t2, strToInt("{AMHeader,0xAA,0x2,0x88,1,0x5,2}"))
axis_handler.read(lmA_t2, 0xAABBCCDD)
axis_handler.read(lmA_t2, 0xDDCCBBAA, tlast=1)
long_message_A.add_thread(lmA_t2)

lmA_t3 = Thread()
axis_tx.read(lmA_t3, strToInt("{AMHeader,0x2,0xAA,0x8,0,0x41,0}"))
