...
axis_net.writes(mmA_t1, [
    {"tdata": strToInt("{AMHeader,0x01,0x02,1,0xE,2,0}"), "callTB": 1},
...
axis_xpams_rx.reads(mmA_t2, [
    {"tdata": strToInt("{AMHeader,0x01,0x02,1,0xE,2,0}")},
...
axis_net.writes(mmB_t1, [
    {"tdata": strToInt("{AMHeader,0x01,0x02,0x100,0xE,2,6}"), "callTB": 1},
...
axis_xpams_rx.reads(mmB_t2, [
    {"tdata": strToInt("{AMHeader,0x01,0x02,0x100,0xE,2,6}")},
...
axis_net.writes(lmA_t1, [
    {"tdata": strToInt("{AMHeader,0x01,0x02,0x100,0xE,4,0}"), "callTB": 1},
...
axis_xpams_rx.read(lmA_t2, strToInt("{AMHeader,0x01,0x02,0x100,0xE,4,0}"))
...
axis_net.writes(lmB_t1, [
    {"tdata": strToInt("{AMHeader,0x01,0x02,0xC,0xD,0x5,2}"), "callTB": 1},
    {"tdata": strToInt("{AMLongStride,0x100,4,3,0x5}"), "callTB": 1},
...
axis_xpams_rx.read(lmB_t3, strToInt("{AMHeader,0x01,0x02,0xC,0xD,0x5,2}"))
axis_xpams_rx.read(lmB_t3, strToInt("{AMLongStride,0x100,4,3,0x5}"))
...
axis_net.writes(lmC_t1, [
    {"tdata": strToInt("{AMHeader,type:0x6,src:0x01,dst:0x02,payload:0x10C,handler:0xF,args:0}"), "callTB": 1},
    {"tdata": strToInt("{AMLongVector,dst:2,dstSize:0xC,token:0x6}")},
...
axis_xpams_rx.reads(lmC_t3, [
    {"tdata": strToInt("{AMHeader,0x01,0x02,0x10C,0xF,0x6,0}")},
    {"tdata": strToInt("{AMLongVector,2,0xC,0x6}")}
])
for i in range(2):
    for j in range(3):
